...
double lastTime_qc = 0;
double lastErr_qc = 0;
double errSum_qc = 0;
unsigned long startTime_qc;
double faixaTrabalho_qc[3] = {};
...
double compute(double input,double setPoint,double kp,double ki, double kd,double confiVals,int variacao,unsigned long startTime,double LedStep);
...
void calibarLed(int ledPin,int ldrPin, double faixaTrabalho[]){
  ...
  faixaTrabalho[2] = (faixaTrabalho[0] - faixaTrabalho[1])/255;
  
  Serial.print("Faixa de trabalho");
  Serial.println( faixaTrabalho[0]);
  Serial.println( faixaTrabalho[1]);
  Serial.println( faixaTrabalho[2]);
